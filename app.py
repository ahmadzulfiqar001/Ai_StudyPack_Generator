...
saved_material = st.session_state.get("material", "")
...
if generate:
    ...
    try:
        ...
        st.error(f"Could not initialize AI workflow: {exc}")
        st.stop()
    ...
    try:
        with st.spinner("Running AI workflow with Groq..."):
            result = workflow.run(
                subject=subject,
                level=level,
                material=material,
        ...
        st.error(f"Study pack generation failed: {exc}")
        st.stop()
        ...
        with st.expander("Workflow warnings"):
            for item in errors:
                if isinstance(item, dict):
                    stage = item.get("stage", "Unknown stage")
    ...
    with tabs[1]:
        ...
        if content:
            ...
            st.info("Content generation stage was not available.")
    ...
    with tabs[2]:
        ...
        if assessment:
            ...
            st.info("Assessment stage was not available.")
    ...
    with tabs[4]:
        final_pack = result.get("final_pack", "")
        ...
        if final_pack:
            ...
            markdown_bytes = final_pack.encode("utf-8")
            ...
            st.warning("Final study pack was not generated.")
else:
    st.info(
